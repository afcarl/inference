...
class playback_positions(tk.Tk):
    ...
    def color_heatmap_grid(self, marginals):
        for (col,row,direction) in marginals:
            tag = 'heat_map_%d_%d_%s' % (row, col, direction),
            # use a colormap that goes from black (low) to white (high)
            # this isn't a particularly good choice...
            prob = marginals[(col,row,direction)]
            color = prob * 255
            color_string = '#%02x%02x%02x'%(color,color,color)

            self.canvas.itemconfig(tag,
                                   fill=color_string,
                                   outline='gray11')

class playback_positions_live(tk.Tk):
    ...
    def __init__(self, true_positions, observed_positions,
                 estimated_marginals, *args, **kwargs):
        tk.Tk.__init__(self, *args, **kwargs)
        self.wm_title('Robot Demo - Online')
        self.canvas = tk.Canvas(self,
                                width=GRID_WIDTH*CELL_WIDTH,
                                height=(GRID_HEIGHT*CELL_HEIGHT + \
                                        PADDING)*4,
        ...
        self.canvas.create_text( (GRID_WIDTH*CELL_WIDTH/2.,
                                  ...
                                  GRID_HEIGHT*CELL_HEIGHT + PADDING/3.),
                                 text='Estimated position distribution' )
        self.redraw(true_positions, observed_positions, estimated_marginals,
                    200)
    ...
    def redraw(self, true_positions, observed_positions, estimated_marginals,
               delay):
        if len(true_positions) == 0:
            self.destroy()
            #pass
            ...
            estimated_marginal = estimated_marginals.next()
            #if estimated_marginals[0] is not None:
            if estimated_marginal is not None:
                estimated_position = estimated_marginal.get_mode()
                self.move_robot_bottom(estimated_position)
                self.color_heatmap_grid(estimated_marginal)
            else:
                # hide robot
                self.move_robot_bottom((-10, -10))
